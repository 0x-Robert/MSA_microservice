...
"""
1. 애플리케이션이 시작할 때 @app.route()로 데코레이트 된 홤수는 뷰로 등록돼 app.url_map에 저장된다.
2. 엔드포인트와 호출 메소드에 따라 올바른 뷰로 요청이 전달된다.
3. Request 객체는 스레드에 안전하게 스레드-로컬 실행 컨텍스트 내에 생성된다.
4. Response 객체는 클라이언트에 반환할 콘텐츠를 감싸고 있다.
